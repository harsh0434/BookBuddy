...
class NytBookApi {
  static Future<List<NytBookModel>> fetchBooks() async {
    List<NytBookModel> books = [];
    const url =
        'https://api.nytimes.com/svc/books/v3/lists/best-sellers/history.json?api-key=AwK9BIHdASvG6QxDFAjZ0iKGNh6yt5G2';
    final uri = Uri.parse(url);
    final response = await http.get(uri);
    final body = response.body;
    final json = jsonDecode(body);
  ...
  static Future<List<NytBookModel>> getBookstolist() async {
    List<NytBookModel> books1 = await fetchBooks();
    List<NytBookModel> booksWithImages = [];
    ...
    for (int i = 0; i < books1.length; i++) {
      ...
      if (json['items'] != null && json['items'].isNotEmpty) {
        final bookData = json['items'][0];
        booksWithImages.add(NytBookModel.fromMap(bookData));
      } else {
        // If no image found, keep the original book data
        booksWithImages.add(books1[i]);
      }
    }
